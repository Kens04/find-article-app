...
const TodoTabs = ({ unreadTodos, readingTodos, TodayTodos }: TodoTabsProps) => {
  ...
  // 本日のTODOのフィルタリング
  const unreadTodayTodos = unreadTodos.filter((todo) => todo.isToday);
  const readingTodayTodos = readingTodos.filter((todo) => todo.isToday);

  // 本日のTODOのページネーション設定
  const unreadPagination = usePagination({
    total: Math.ceil(unreadTodayTodos.length / PAGINATION.ITEMS_PER_PAGE),
    initialPage: 1,
  });

  const readingPagination = usePagination({
    total: Math.ceil(readingTodayTodos.length / PAGINATION.ITEMS_PER_PAGE),
    initialPage: 1,
  });
  ...
  // カテゴリーでフィルタリングした本日のTODOを取得
  const filteredUnreadTodos = getSortedTodos(
    selectedCategories.length === 0
      ? unreadTodayTodos
      : unreadTodayTodos.filter((todo) =>
  ...
  const unreadFiltered = unreadTodayTodos.filter((todo) =>
    todo.title.toLowerCase().includes(search?.toLowerCase() || "")
  );
  const readingFiltered = readingTodayTodos.filter((todo) =>
    todo.title.toLowerCase().includes(search?.toLowerCase() || "")
  );
